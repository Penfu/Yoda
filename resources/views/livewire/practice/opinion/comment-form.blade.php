<div class="flex py-4">
    @auth
        <form wire:submit.prevent="post" class="grow group">
            @csrf
            @error('comment')
                <span class="pt-2 pb-1 px-2 bg-red-200 rounded shadow font-medium italic">{{ $message }}</span>
            @enderror

            <div class="grow group flex flex-col sm:flex-row items-end space-y-2 sm:space-x-2">
