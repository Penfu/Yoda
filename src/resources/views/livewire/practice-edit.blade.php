    ...
    @if ($isEditing)
        <form wire:submit.prevent="editTitle">
            <div class="py-4">
                <div class="w-full">
                    @error('title')
                        <span class="pt-2 pb-1 px-2 bg-red-200 rounded shadow text-red-500 font-medium italic">{{ $message }}</span>
                    @enderror
                    <label for="title" class="block">Titre</label>
                    <input type="text" wire:model="title" name="title" id="title" required autofocus autocomplete="off" value="{{ $practice->title }}"
