<div class="outline outline-4 outline-gray-200 rounded">
    <button wire:click="open" class="accordion-btn w-full px-4 bg-gray-200 rounded-t text-left cursor-pointer">
        <span class="block py-1 font-bold text-2xl">
            <span>{{ $comments->count() }}</span>
            <h3 class="inline uppercase">{{ $comments->count() > 1 ? 'Commentaires' : 'Commentaire' }}</h3>
            <svg xmlns="http://www.w3.org/2000/svg" class="accordion-icon float-right inline-block h-8 w-8 {{ $isOpen ? 'rotate-180' : null }}" fill="none"
    ...
    <div class="accordion-content {{ $isOpen ? null : 'hidden' }}
     px-4 divide-y-4 divide-gray-200">
        ...
        @endforeach
        <livewire:practice.opinion.comment-form :opinion="$opinion" />
    </div>
</div>
