...
class PracticeSeeder extends Seeder
{
    ...
    public function run()
    {
        ...
        $practices = [
            ...
            ['title' => 'Les globaux sont mauvais', 'description' => "Les globaux sont mauvais. Les fonctions sont meilleures que les types. Les objets sont probablement meilleurs que les structures de données complexes."],
            ['title' => 'Types intégrés', 'description' => "L'utilisation des types intégrés de Python - et de leurs méthodes - sera plus rapide que l'écriture de vos propres types (sauf si vous écrivez en C). Si les performances sont un facteur important, essayez de trouver un moyen d'utiliser les types intégrés standard plutôt que des objets personnalisés."],
            ['title' => 'Injection de dépendances', 'description' => "L'injection de dépendances est un modèle de codage utile pour être clair sur ce que sont vos dépendances et d'où elles viennent. (Faites en sorte que les objets, les méthodes, etc. reçoivent leurs dépendances en tant que paramètres plutôt que d'instancier eux-mêmes de nouveaux objets). Cela rend les signatures d'API plus complexes, c'est donc un compromis. Si vous vous retrouvez avec une méthode qui nécessite 10 paramètres pour toutes ses dépendances, c'est un bon signe que votre code en fait trop, de toute façon. L'article définitif sur l'injection de dépendances est \"Inversion of Control Containers and the Dependency Injection Pattern\"], par Martin Fowler."],
            ['title' => 'Peu de simulations', 'description' => "Plus vous devez faire des simulations pour tester votre code, plus votre code est mauvais. Plus vous devez instancier et mettre en place du code pour pouvoir tester un comportement spécifique, plus votre code est mauvais. L'objectif est de créer de petites unités testables, ainsi que des tests d'intégration et fonctionnels de plus haut niveau pour vérifier que les unités coopèrent correctement."],
            ['title' => 'API externes', 'description' => "Les API externes sont celles pour lesquelles la \" conception en amont \" - et la prise en compte des cas d'utilisation futurs - est vraiment importante. Changer d'API est pénible pour nous et pour nos utilisateurs, et créer une incompatibilité rétroactive est horrible (bien que parfois impossible à éviter). Concevez les API externes avec soin, tout en respectant le principe \" les choses simples doivent rester simples \"."],
            ['title' => '30 lignes max', 'description' => "Si une fonction ou une méthode dépasse 30 lignes de code, envisagez de la décomposer. Une bonne taille maximale de module est d'environ 500 lignes. Les fichiers de test ont tendance à être plus longs que cela."],
            ['title' => 'Pas de job dans les constructeurs', 'description' => "Ne faites pas de travail dans les constructeurs d'objets, qui sont difficiles à tester et surprenants. Ne mettez pas de code dans __init__.py (sauf les importations pour l'espacement des noms). __init__.py n'est pas l'endroit où les programmeurs s'attendent généralement à trouver du code, donc c'est \"surprenant\"."],
            ['title' => 'DRY', 'description' => "DRY (Don't Repeat Yourself) importe beaucoup moins dans les tests que dans le code de production. La lisibilité d'un fichier de test individuel est plus importante que la maintenabilité (décomposer des morceaux réutilisables). Cela s'explique par le fait que les tests sont exécutés et lus individuellement plutôt que de faire partie d'un système plus vaste. Évidemment, une répétition excessive signifie que des composants réutilisables peuvent être créés pour des raisons de commodité, mais c'est beaucoup moins une préoccupation que pour la production."],
            ['title' => 'Refactorisez souvent', 'description' => "Refactorisez chaque fois que vous en voyez le besoin et que vous en avez l'occasion. La programmation est une question d'abstractions, et plus vos abstractions sont proches du domaine du problème, plus votre code est facile à comprendre et à maintenir. Au fur et à mesure que les systèmes se développent organiquement, ils doivent changer de structure pour s'adapter à l'expansion de leurs cas d'utilisation. Les systèmes dépassent leurs abstractions et leur structure, et ne pas les changer devient une dette technique qui est plus pénible (et plus lente et plus boguée) à gérer. Incluez le coût de l'élimination de la dette technique (remaniement) dans les estimations du travail sur les fonctionnalités. Plus vous laissez la dette en place, plus les intérêts s'accumulent. Un excellent livre sur le remaniement et les tests est Working Effectively with Legacy Code, de Michael Feathers."],
            ['title' => 'D\'abord juste, efficace après', 'description' => "Faites en sorte que le code soit d'abord correct et ensuite rapide. Lorsque vous travaillez sur des problèmes de performance, établissez toujours un profil avant d'apporter des corrections. En général, le goulot d'étranglement n'est pas tout à fait là où vous pensiez qu'il était. Écrire du code obscur parce qu'il est plus rapide ne vaut la peine que si vous l'avez profilé et prouvé que cela en vaut vraiment la peine. Écrire un test qui exerce le code que vous profilez avec un timing autour de lui permet de savoir plus facilement quand vous avez fini, et peut être laissé dans la suite de tests pour prévenir les régressions de performance. (Avec la remarque habituelle que l'ajout de code de chronométrage change toujours les caractéristiques de performance du code, ce qui fait du travail de performance l'une des tâches les plus frustrantes)."]
        ];
